...
try
{
    ...
    if (env != "hes")
    {
        ...
        string dbEnv = env == "dev" ? "staging" : env;
        var secretName = $"{dbEnv}/{appName}/rds/connection-strings__SqlServerConnection";
        var secret = await secretManagerHelper.GetSecretValueAsync(secretName);
        
        (appConnectionString, logConnectionString) = GetConnectionString(dbEnv, secret);
        var telegramBotSecret = await secretManagerHelper.GetSecretValueAsync($"{dbEnv}/{appName}/telegram-bot/token");
...
(string appConnection, string logConnection) GetConnectionString(string env, string secretString)
{
    ...
    var appDatabaseName = env == "prod" ? "RRM" : "BrainHunter";
    var logDatabaseName = env == "prod" ? "RRM" : "BrainHunter";
